...
class StixCoreRelationship:
    ...
    @staticmethod
    def generate_id(relationship_type,
                    source_ref,
                    target_ref,
                    start_time=None,
                    stop_time=None):
    ...
    def list(self, **kwargs):
        ...
        if get_all:
            ...
            while result["data"]["stixCoreRelationships"]["pageInfo"][
                    "hasNextPage"]:
                after = result["data"]["stixCoreRelationships"]["pageInfo"][
                    "endCursor"]
                self.opencti.app_logger.info("Listing StixCoreRelationships",
                                             {"after": after})
                result = self.opencti.query(
                    query,
                    {
    ...
    def read(self, **kwargs):
        ...
        custom_attributes = kwargs.get("customAttributes", None)
        if id is not None:
            self.opencti.app_logger.info("Reading stix_core_relationship",
                                         {"id": id})
            query = ("""
    ...
    def update_field(self, **kwargs):
        ...
        if id is not None and input is not None:
            self.opencti.app_logger.info("Updating stix_core_relationship",
                                         {"id": id})
            query = """
    ...
    def delete(self, **kwargs):
        ...
        id = kwargs.get("id", None)
        if id is not None:
            self.opencti.app_logger.info("Deleting stix_core_relationship",
                                         {"id": id})
            query = """
            ...
            return self.create(
                ...
                description=(self.opencti.stix2.convert_markdown(
                    stix_relation["description"])
                             if "description" in stix_relation else None),
                start_time=(stix_relation["start_time"] if "start_time"
                            in stix_relation else default_date),
                stop_time=(stix_relation["stop_time"]
                           if "stop_time" in stix_relation else default_date),
                revoked=(stix_relation["revoked"]
                         if "revoked" in stix_relation else None),
                confidence=(stix_relation["confidence"]
                            if "confidence" in stix_relation else None),
                lang=stix_relation["lang"]
                if "lang" in stix_relation else None,
                created=(stix_relation["created"]
                         if "created" in stix_relation else None),
                modified=(stix_relation["modified"]
                          if "modified" in stix_relation else None),
                ...
                objectOrganization=(stix_relation["x_opencti_granted_refs"]
                                    if "x_opencti_granted_refs"
                                    in stix_relation else None),
                x_opencti_workflow_id=(stix_relation["x_opencti_workflow_id"]
